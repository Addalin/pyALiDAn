...
class Bezier:
    ...
    @staticmethod
    def _get_bezier_cubic(points):
        """
        Returns one cubic curve for each consecutive points
        :param points:
        :return:
        """
        A, B = Bezier._get_bezier_coef(points)
        return [
            Bezier._get_cubic(points[i], A[i], B[i], points[i + 1])
    ...
    def evaluate_bezier(points, n):
        ...
        :return:
        """
        curves = Bezier._get_bezier_cubic(points)
    ...
    def bezier_example_usage():
        ...
        path = Bezier.evaluate_bezier(points, 50)
