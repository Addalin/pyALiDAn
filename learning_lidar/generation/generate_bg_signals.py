...
fig, ax = plt.subplots(ncols=1,nrows=1)
for i,(curve_h,curve_l,mean_val,c,chan,wavelength) in         enumerate(zip(high_curves,low_curves,mean_curves,vis_utils.COLORS,['UV','G','IR'],wavelengths)):
...
mean_new_curves = []
mean_new_signal = []
for i,(curve_h,curve_l,mean_val,c,chan) in         enumerate(zip(high_curves,low_curves,mean_curves,vis_utils.COLORS,['UV','G','IR'])):
	prms_h = params[chan]['high']
	prms_l = params[chan]['low']
...
nc_name = f"generated_bg_{station.name}_"           f"{start_day.strftime('%Y-%m-%d')}_{end_day.strftime('%Y-%m-%d')}.nc"
